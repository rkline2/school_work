...
void neighborsValidation(int board2d[][4], int row, int col, int validNeighbor[4]);
const int ABOVE = 0;
const int RIGHT = 1;
const int BELOW = 2;
const int LEFT = 3;
...
void playGame(int board2d[][MAX_COL], int board1d[16], int zero[COORDINATES], int validNeighbor[TOTAL_NEIGHBORS])
{
        int row, col = CLEAR;
        ...
        while(GAME_OVER == false)
        {
                ...
                neighborsValidation(board2d, row, col, validNeighbor);

                //move the game piece
                board2d[row][col] = board2d[zero[0]][zero[1]];
                board2d[zero[0]][zero[1]];
...
void neighborsValidation(int board2d[][MAX_COL], int row, int col, int validNeighbor[TOTAL_NEIGHBORS])
{
        bool input = false;

        while(input == false)
        {
                if(board2d[row-1][col-1] == validNeighbor[ABOVE] ||
                board2d[row-1][col-1] == validNeighbor[RIGHT] ||
                board2d[row-1][col-1] == validNeighbor[LEFT] || 
                board2d[row-1][col-1] == validNeighbor[BELOW])
                {
                        input = true;
                }
...
void findZero(int board2d[][MAX_COL], int board1d[16], int zero[COORDINATES], int validNeighbor[TOTAL_NEIGHBORS])
{
...
void createBoard(string testFile, int board1d[16], int board2d[][MAX_COL])//char testFile[])
{
        int num = 0, k = 0;
